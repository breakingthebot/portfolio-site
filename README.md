...
### SEO basics: page titles, robots.txt, sitemap
Every page sets its own browser tab title via `useDocumentTitle()` (`src/hooks/useDocumentTitle.js`) — previously every route showed the same static title from `index.html`, so tabs/history/bookmarks couldn't tell pages apart. `public/robots.txt` allows all crawlers and points at `sitemap.xml`, which is generated fresh before every build (`scripts/generate-sitemap.js`, wired in via the `prebuild` npm script) from the real route list plus every real blog post slug — so it's never hand-maintained and can't drift out of sync as posts are added. `public/sitemap.xml` itself is gitignored since it's a build artifact, not source.

**Known limitation**: Open Graph/Twitter Card preview tags in `index.html` are still site-wide, not per-page — sharing a link to a specific blog post shows the homepage's preview card, not one about that post. This is a harder problem than the tab title: social media crawlers read the static HTML as served and don't execute JavaScript, so a client-side fix (like the title) doesn't reach them. Fixing it properly would need per-route static HTML (a prerender step) or a Vercel Edge Function rewriting meta tags per request — not done here, since it's a real infrastructure addition, not a quick fix.

Home also has a tech-stack badge row (`src/components/TechStack.jsx`, content in `src/content/techStack.js`) and a recent-activity heatmap (`src/components/GithubActivity.jsx`, logic in `src/services/githubActivityService.js`). This originally embedded a third-party image (`ghchart.rshah.org`), but that turned out to be several+ days stale and, as a plain `<img>`, couldn't expose per-day data for tooltips at all — so it was replaced with a first-party widget built from GitHub's own public events API (CORS-enabled, no auth needed for public data). Two real constraints shaped it:
- **No commit counts available.** GitHub's public `PushEvent` payload doesn't include a commit count or commit list (confirmed against the live API — only `repository_id`/`push_id`/`ref`/`head`/`before`). Getting an exact count would mean comparing SHAs via a separate API call per push, which isn't safe against the 60-request/hour unauthenticated rate limit. The widget counts and labels **pushes**, not commits, to stay accurate.
- **Limited history.** The events endpoint caps at 300 events across 10 pages, all fetched in parallel on mount — during an active stretch that's only a few weeks of real coverage, not the full 91-day window the grid displays; older cells are often genuinely empty rather than a bug. The unauthenticated rate limit is scoped per visitor IP, not shared site-wide, so fetching all 10 pages per page load is safe for normal traffic.

If every page fetch fails (offline, rate-limited), the widget hides itself entirely rather than showing an error or stale placeholder.
